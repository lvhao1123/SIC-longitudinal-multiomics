...
for(tm in names(entries)) {
  ...
  full$status<-ifelse(!full$eligible,'coverage_outside_original_15_to_500',ifelse(rep(nzchar(gerr),nrow(full)),'GSEA_call_failed',ifelse(!is.finite(full$NES)|!is.finite(full$pval),'result_uncertain_nonfinite',ifelse(!is.finite(full$log2err),'estimable_precision_uncertain','estimable'))))
  gsea_out[[tm]]<-full
